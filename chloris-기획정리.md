...
- 보로는 **S3 전환 완료**(2026-06-15, Supabase Storage `boro-uploads`). 새 업로드는 S3, 기존 inline 그대로. **자동 이미지 압축 코어 적용 완료**(`lib/imageCompress.js`).
...
- 보로 이미지 압축·S3 전환 완료(2026-06-15).
- 다음: CRM·예약 모듈(보로 전용) → 인프라 분리(원모먼트·오늘꽃 콘솔 작업).
