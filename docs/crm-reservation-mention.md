...
1. **트리거**: ① "예약" 버튼(v1, 빠름) / ② `@예약` 멘션(v2, 코어 멘션 확장 필요) / ③ 버튼 먼저 → 멘션 나중.
   - 제안: **③** (버튼으로 현장 가치 즉시 + 멘션은 에이전트 플랫폼과 함께).
2. **폼 표시**: 모달 vs 인라인 패널. (제안: 모달)
3. **활성 채널**: branchId 연결된 채널만 노출 vs 모든 채널(지점 수동 선택). (제안: 연결된 채널만)
4. **채널 요약 카드**: 제출 후 #지점방에 요약 남길지. (제안: 켬 — 가시성)
5. **권한**: 현장 직원 누구나 vs 역할 제한. (제안: 인증 사용자 누구나, CRM 모듈 활성 브랜드 한정)
