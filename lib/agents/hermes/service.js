...
export async function runHermesAgent({ body, channelId, messageId, requester }) {
  ...
  try {
    // 분류는 자체 try/catch로 감싼다 — 키없음·파싱실패·API/네트워크 throw 등 어떤 실패든 area=null로 안전 degrade.
    let area = null;
    try {
      const cls = await classifyJson({ messages: buildWorkIntentMessages(stripHermesAgentMention(body)) });
      area = cls?.ok ? (cls.data?.area ?? null) : null;
    } catch {
      area = null;
    }
    ...
    const action = canRoute ? "route" : "help";
    const lines = canRoute ? buildRouteMessageLines(route) : HERMES_HELP_LINES;
