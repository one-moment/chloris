...
function isLikelyMetadataLineBeforeVendor(rawLine) {
  const cleaned = cleanLine(rawLine);
  if (!cleaned) return true;
  if (/^\[[^\]]+\]/u.test(cleaned)) return true;
  if (/^\d{4}[-./]\d{1,2}[-./]\d{1,2}/u.test(cleaned)) return true;
  if (/^(제목|테스트|운영\s*테스트|요청|메모)\s*[:：]/u.test(cleaned)) return true;
  return !String(rawLine ?? "").trim().startsWith("-");
}
...
export function parseBulkPurchaseOrder(body) {
  ...
  for (const rawLine of rawLines) {
    ...
    const cleaned = cleanLine(rawLine);
    const urls = [...cleaned.matchAll(URL_PATTERN)].map((match) => match[1]);
    if (!currentVendor && urls.length === 0 && isLikelyMetadataLineBeforeVendor(rawLine)) continue;
