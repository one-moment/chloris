...
export async function notifyMention({ recipientIds = [], authorId, authorName, channelId, body } = {}) {
  ...
  if (!isPushConfigured()) {
    console.log("[push] mention: skip — push not configured");
    return { configured: false, targets: 0, sent: 0 };
  }

  const targets = recipients.filter((id) => id && id !== authorId);
  if (targets.length === 0) {
    console.log(`[push] mention: skip — 0 targets (recipients=${recipients.length}, authorInRecipients=${recipients.includes(authorId)})`);
    return { configured: true, targets: 0, sent: 0 };
  }
  ...
  console.log(`[push] mention: targets=${targets.length} sent=${sent} removed=${removed} failed=${failed}`);
