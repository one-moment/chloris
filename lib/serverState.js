...
export async function readState() {
  ...
  const [projects, bots, users] = await Promise.all([
...
export async function writeState(state) {
  await prisma.$transaction(async (tx) => {
    // ChannelMember 는 클라이언트 state 에 포함되지 않으므로 재생성 단계에서 복원되지 않는다.
    // channel.deleteMany() 시 onDelete:Cascade 로 함께 삭제되기 때문에, 먼저 스냅샷해 두었다가
    // 동일 id 로 채널을 재생성한 뒤 복원한다.
    const memberSnapshot = await tx.channelMember.findMany();
    ...
    // 동일 id 로 재생성된 채널에 한해 ChannelMember 를 복원한다.
    // (state 에서 사라진 채널의 멤버는 채널과 함께 정리된 것으로 보고 복원하지 않는다.)
    const restoredChannelIds = new Set(
      (state.projects ?? []).flatMap((project) => (project.channels ?? []).map((channel) => channel.id))
    );
    for (const member of memberSnapshot) {
      if (!restoredChannelIds.has(member.channelId)) continue;
