...
export function serializeDisposalBatch(batch) {
  let attachments = [];
  try {
    const parsed = JSON.parse(batch.attachmentsJson ?? "[]");
...
// 전 지점 매니저(isAllBranches=true, branchId=null)는 모든 지점의 매니저 풀/승인권에 포함된다(PLAN §4).
// early-return(branchId 없으면 []) 은 그대로 두고 where절만 OR로 확장(소비처는 항상 branchId 전달).
export async function branchManagers(branchId) {
  if (!branchId) return [];
  const assignments = await prisma.branchAssignment.findMany({
    where: { role: "manager", OR: [{ branchId }, { isAllBranches: true }] },
...
// 폼은 branchId로 필터(DisposalDashboard.jsx)하므로, 전 지점 매니저(isAllBranches=true, branchId=null)는
// 활성 지점마다 1행으로 전개해 모든 지점 드롭다운에 노출시킨다(PLAN §4 — reviewer 드롭다운 노출).
export async function allBranchManagers() {
  const [assignments, branches] = await Promise.all([
    prisma.branchAssignment.findMany({
      where: { role: "manager" },
      select: { branchId: true, isAllBranches: true, user: { select: { id: true, name: true, handle: true } } }
    }),
    prisma.branch.findMany({ where: { status: "active" }, select: { id: true } })
  ]);
  const result = [];
  for (const assignment of assignments) {
    if (!assignment.user) continue;
    const person = { id: assignment.user.id, name: assignment.user.name, handle: assignment.user.handle };
    if (assignment.isAllBranches) {
      for (const branch of branches) result.push({ branchId: branch.id, ...person });
    } else if (assignment.branchId) {
      result.push({ branchId: assignment.branchId, ...person });
    }
  }
  return result;
}

// 해당 지점 폐기건을 승인/반려할 수 있는가: 전역 admin 이거나 그 지점(또는 전 지점) 매니저.
// early-return 분기는 유지하고 where절만 OR로 확장(전 지점 매니저는 branchId 무관 승인).
export async function canApproveDisposal(user, branchId) {
  ...
  const assignment = await prisma.branchAssignment.findFirst({
    where: { userId: user.id, role: "manager", OR: [{ branchId }, { isAllBranches: true }] },
    select: { id: true }
  });
