...
- [~] Borough feature build started (track A first; infra split deferred by user):
  ...
  - [~] CRM + reservations (Borough-only modules) per `docs/templates-and-crm.md` — IN PROGRESS (Ralph loop).
    ...
    - [ ] (3) `/work/customers` (search/profile/history/manual entry) + `/work/reservations` (list/calendar/HQ rollup). — next (replace stub dashboards; customers screen consumes the lookup API)
