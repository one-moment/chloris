...
### Phase G0 — 정산 데이터 모델 + 대조 로직(순수) ✅(코드, 2026-06-20)
Prisma 4모델(양쪽 스키마, additive·enum 미사용·String id): `PurchaseInvoice`·`CollectionJob`·`BankDeposit`·`DepositorAlias`. `lib/reconcile.js` 순수 대조(매입 1:1 CONFIRMED/AMBIGUOUS, 입금 부분누적·별칭귀속·잔여계산, 외부의존 0).
- **Done when**: 모델 양쪽 동일 + 로컬 sqlite 표 생성 + 합성검증(12케이스) + lint/build. ✅ (커밋 `1e8b4c5`) — 매입(G)·입금(H) 로직 토대 공용.

---

## 예정 Phase

### Phase G — 오늘꽃 정산: 단계 1 (매입 계산서 대조) — 연동 잔여
✅ 토대(`PurchaseInvoice`/`CollectionJob` + `reconcilePurchaseInvoices`)는 Phase G0 완료. **잔여**: 변환층(팝빌 매입 응답 → reconcile 입력 규격), settlement API(수집/폴링/대조/목록, 기존 `app/api/work/*` 패턴), 시트 "계산서 확인" 되써넣기(`lib/googleSheets.js` 재사용).
- **Done when**: 실제 사입 건에 "계산서 확인 ✓/확인 필요"가 시트+앱에 자동·정확 표시(테스트베드). 송금은 사람.
- ⚠️ **블로커**: 거래원장 시트가 아직 운영 자동화로 **미확정** → 칸 구성 미확정 → 변환층·되써넣기 설계 보류.

### Phase H — 오늘꽃 정산: 단계 2 (입금 대조 + 정산) — 연동 잔여
✅ 토대(`BankDeposit`/`DepositorAlias` + `reconcileDeposits` 부분누적·별칭·잔여)는 Phase G0 완료. **잔여**: 변환층(계좌조회 응답 → 입력 규격), 입금 수집 API, 별칭 관리 API, 시트 되써넣기.
- **Done when**: 입금/미입금/잔여금액 자동·정확 표시 + 별칭 1회 연결 후 자동 매칭.
- ⚠️ **블로커**: Phase G와 동일(거래원장 시트 확정 대기).
...
- ★ **거래원장 시트 확정(최우선 선행)**: 거래원장 **자동화 시트가 아직 운영에 반영 전**이라 거래원장을 확정할 수 없음 → 사입·납품 **칸 구성(제목 행)** 제공 불가. 이게 풀려야 Phase G·H '연동'을 진행한다.
- 팝빌 가입+테스트베드 / 홈택스 부서사용자 계정→팝빌 등록 / 입금 은행계좌 팝빌 등록 / 구글 서비스계정 시트 편집공유 / 연동 키 `.env`. (월 거래량·계좌 수 미확인 — 추정)
